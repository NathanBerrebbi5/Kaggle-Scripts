...
plt.figure(1)
pd.Series(df["airline_sentiment"]).value_counts().plot(kind = "pie" , title = "airline_sentiment" , autopct='%.2f')
plt.figure(2)
pd.Series(df["airline"]).value_counts().plot(kind = "bar" , title = "airline" , table = True , xticks = [])
plt.figure(3)
...
plt.figure(12)
sns.distplot(df["airline_sentiment_confidence"])
...
pd.crosstab(index =df["airline_sentiment"] ,  columns = df["dayofweek"]  ).plot(kind = "pie", subplots = True ,  autopct='%.2f' )
pd.crosstab(index = df["hour"] ,  columns = df["airline_sentiment"] ).plot(kind = "barh" , stacked = True)
pd.crosstab(index = df["user_timezone"] ,  columns = df["airline_sentiment"] ).head(10).plot(kind = "barh" , stacked = True)
...
def cleantext2(review):
    review = BeautifulSoup(review ,"lxml").get_text()
    review_words = cleanword(review.lower()).split()    
    stop = stopwords.words('english')
    res = [w for w in review_words if w not in stop]
    return " ".join(res)


sent =   "GSW2015!Winners rasta! so tomatoes"
print cleantext(sent)
df["text"]  = df["text"].apply(cleantext)
#df["text"]  = df["text"].apply(cleantext2)
...
ee =  remove_short(all_text).split()
ff = " ".join(ee)
#finder = BigramCollocationFinder.from_words(remove_short(ff))
#rrr = finder.nbest(bigram_measures.pmi, 10)

tokens = nltk.word_tokenize(ff)
bgs = nltk.bigrams(tokens)
fdist = nltk.FreqDist(bgs)
fdist_list = []
for k,v in fdist.items():  
    fdist_list.append((k,v))
    
fdist_list.sort(key = lambda x:-x[1])
print fdist_list[:15]
